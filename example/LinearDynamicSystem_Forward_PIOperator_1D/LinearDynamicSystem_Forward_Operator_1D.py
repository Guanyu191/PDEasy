...
# --- DeepONet 模型 ---
# ---------------------
class PIDeepONet(PIDeepONetForward):
    ...
    def forward(self, data_dict):
        ...
        loss_dict = {}
        loss_dict['pw_loss_res'] = self.net_res(F, X) ** 2
        loss_dict['pw_loss_ics'] = self.net_sol(F, torch.tensor([[0.]], device=self.device)) ** 2
        loss_dict['pw_loss_obs'] = (self.net_sol(F, X) - U) ** 2
        return loss_dict
    
    def net_res(self, F, X):
...
deeponet = PIDeepONet(network_branch, network_trunk)
...
log_keys = ['iter', 'loss', 'loss_res', 'loss_ics', 'loss_obs', 'loss_test']
...
for it in range(N_ITERS):
    ...
    loss = loss_res + loss_ics + loss_obs
