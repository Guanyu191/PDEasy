...
def generate_dataset_for_pideeponet(
    ...
    """
    _F.shape = (n_func, n_sensor)
    _X.shape = (n_func, n_loc)
    _U.shape = (n_func, n_loc)
    ...
    for i in range(n_func):
        f = _F[i]  # (n_sensor, ) represent a function
        x = _X[i]  # (n_loc, )
        u = _U[i]  # (n_loc, )

        F.append(np.repeat(f.reshape(1, -1), n_loc, axis=0))
        X.append(x.reshape(-1, 1))
        U.append(u.reshape(-1, 1))

        f_interp = interp1d(x_sensor, f, kind='cubic', fill_value="extrapolate")
        f_res = f_interp(x)
        F_res.append(f_res.reshape(-1, 1))

    F = np.concatenate(F, axis=0)
    X = np.concatenate(X, axis=0)
    U = np.concatenate(U, axis=0)
    F_res = np.concatenate(F_res, axis=0)
    return F, X, U, F_res
...
if __name__ == '__main__':
    ...
    # Trainset
    F, X, U, F_res = generate_dataset_for_pideeponet(
        g, DOMAIN, N_FUNC, N_SENSOR, LENGTH_SCALE, N_LOC
    )
    savemat(
        os.path.join(DATA_DIR, 'HighFreqAntiDerivativeOperator_Sol_Trainset.mat'),
        {'F': F, 'X': X, 'U': U, 'F_res': F_res}
    )

    # Testset
    F, X, U, F_res = generate_dataset_for_pideeponet(
        g, DOMAIN, N_FUNC_TEST, N_SENSOR, LENGTH_SCALE, N_LOC_TEST
    )
    savemat(
        os.path.join(DATA_DIR, 'HighFreqAntiDerivativeOperator_Sol_Testset.mat'),
        {'F': F, 'X': X, 'U': U, 'F_res': F_res}
    )