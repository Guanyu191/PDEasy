r'''
Descripttion: Example of operator learning on 1D high-frequency anti-derivative operator.
Author: Guanyu
...
from pdeasy.framework import PIDeepONetForward
...
N_ITERS = 300000
NN_BRANCH_LAYERS = [N_SENSOR] + [200]*5
NN_TRUNK_LAYERS = [N_DIM] + [200]*5
...
trainset = io.loadmat(os.path.join(DATA_DIR, 'HighFreqAntiDerivativeOperator_Sol_Trainset.mat'))
F_train = trainset['F']  # (N_FUNC * N_LOC, N_SENSOR)
X_train = trainset['X']  # (N_FUNC * N_LOC, N_DIM)
U_train = trainset['U']  # (N_FUNC * N_LOC, 1)
testset = io.loadmat(os.path.join(DATA_DIR, 'HighFreqAntiDerivativeOperator_Sol_Testset.mat'))
F_test = testset['F']
X_test = testset['X']
U_test = testset['U']
...
network_trunk = MFF1D(NN_TRUNK_LAYERS, act_type='tanh', sigma_1=1, sigma_2=50)
...
# --------------------------
io.savemat(
    os.path.join(DATA_DIR, 'HighFreqAntiDerivativeOperator_Sol_Testset_PIDeepONet.mat'),
