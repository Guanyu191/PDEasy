r'''
Descripttion: Example of operator learning on 1D anti-derivative operator.
Author: Guanyu
...
trainset = io.loadmat(os.path.join(DATA_DIR, 'AntiDerivativeOperator_Sol_Trainset.mat'))
F_train = trainset['F']  # (N_FUNC * N_LOC, N_SENSOR)
X_train = trainset['X']  # (N_FUNC * N_LOC, N_DIM)
U_train = trainset['U']  # (N_FUNC * N_LOC, 1)
testset = io.loadmat(os.path.join(DATA_DIR, 'AntiDerivativeOperator_Sol_Testset.mat'))
F_test = testset['F']
X_test = testset['X']
U_test = testset['U']
...
log_keys = ['iter', 'loss', 'loss_operator', 'loss_physics', 'error']
...
# --------------------------
io.savemat(
    os.path.join(DATA_DIR, 'AntiDerivativeOperator_Sol_Testset_PIDeepONet.mat'),
