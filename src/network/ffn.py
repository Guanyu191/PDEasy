...
class MFF1D(nn.Module):
    ...
    模型说明：
        nn_layers[0] 是原始的 [x] 输入
        [x] 经过 Fourier Embedding 变为 H_1 和 H_2
...
class FF2D(nn.Module):
    r"""
    Fourier Feature Model for 2D Space
    模型说明：
        nn_layers[0] 是原始的 [x, y] 输入
        [x, y] 经过 Fourier Embedding 变为 H_x 和 H_y
        其中 W_x 和 W_y 是 Fourier Embedding 的参数
        进而 H_x 和 H_y 先后传入同一个网络 (例如 MLP)
        最后将 H = H_x * H_y 作为融合特征传给最后的线性层返回 u
    ---
    参数说明：
        nn_layers: 神经网络的层结构
            例如 [2, 100, 100, 100, 1]
...
class MFF2D(nn.Module):
    r"""
    Multiscale Fourier Feature Model for 2D Space
    模型说明：
        nn_layers[0] 是原始的 [x, y] 输入
        [x, y] 经过 Fourier Embedding 变为 H_x_1, H_x_2 和 H_y_1, H_y_2
        其中 W_x_1, W_x_2 和 W_y_1, W_y_2 是 Fourier Embedding 的参数
        进而 H_x_1, H_x_2 和 H_y_1, H_y_2 先后传入同一个网络 (例如 MLP)
        最后将 H = [H_x_1, H_x_2, H_y_1, H_y_2] 作为拼接特征传给最后的线性层返回 u
    ---
    参数说明：
        nn_layers: 神经网络的层结构
            例如 [2, 100, 100, 100, 1]
    ...
    """
    def __init__(self, nn_layers, act_type='tanh', init_type='default',
                 sigma_x_1=1, sigma_x_2=10, sigma_y_1=1, sigma_y_2=10):
...
class FF2DT(nn.Module):
    r"""
    Fourier Feature Model for 2D Space-Time
    模型说明：
        nn_layers[0] 是原始的 [x, y, t] 输入
        [x, y, t] 经过 Fourier Embedding 变为 H_x, H_y, H_t
        其中 W_x, W_y, W_t 是 Fourier Embedding 的参数
        进而 H_x, H_y, H_t 先后传入同一个网络 (例如 MLP)
        最后将 H = H_x * H_y * H_t 作为融合特征传给最后的线性层返回 u
    ---
    参数说明：
        nn_layers: 神经网络的层结构
            例如 [3, 100, 100, 100, 1]
    ...
    """
    def __init__(self, nn_layers, act_type='tanh', init_type='default',
                 sigma_x=10, sigma_y=10, sigma_t=10):
        super(FF2D, self).__init__()
